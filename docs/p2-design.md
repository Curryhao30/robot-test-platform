...
> 前置：P0（v0.1.0-p0）+ P1（v0.2.0-p1 ~ v0.7.0-p1）+ **P2.0（v0.9.0-p2.0）** +
> **P2.1a（v0.10.0-p2.1a）** + **P2.2a（v0.11.0-p2.2a）** +
> **P2.3（v0.12.0-p2.3，已完成）**，74 用例双端全绿。
...
| P2.1 | **真 EtherCAT HIL**：SOEM 主站替换虚拟总线（**P2.1a 抽象+骨架 ✅**；
P2.1b 接 SOEM 扫描/PDO） | P2.0 | 1× 工控机/树莓派 + 1× 伺服驱动器（如支持 CiA402） |
| P2.2 | **FSoE 安全联锁**：安全逻辑仿真（**P2.2a ✅**）→
真实安全 I/O（P2.2b，需硬件） | P2.1 | 安全 PLC/安全继电器或 FSoE 安全端子 |
| P2.3 | **真示教器接入**：真实示教器协议桥（**协议模拟器 ✅**；
RealTPBridge 待厂商资料） | P2.1 | 华沿/第三方示教器（协议需厂商资料，见 §5） |
...
### 4.4 验收（P2.1a 部分 ✅：抽象/骨架/清晰失败）
1. 同一份 `test_ethercat.py` 在真机模式下通过（slave 数匹配）——待 P2.1b；
   已达成：`--bus virtual|soem` 切换、SOEM 骨架参数校验/网卡探测/
   Windows 明确不支持、无网卡 FATAL 清晰退出（3 用例）；
...
| 7 | 示教器协议模拟器（✅ v0.12.0-p2.3） | 纯软件，已完成 |
